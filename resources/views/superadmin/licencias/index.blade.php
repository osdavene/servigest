...
<div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 32px;">
        @forelse($planes as $plan)
            <div class="card" style="display: flex; flex-direction: column; justify-content: space-between; border-color: {{ $plan->talleres_count > 0 ? '#fde68a' : 'var(--border)' }}; margin-bottom: 0;">
                <div>
                    ...
                    <strong style="font-size: 18px; color: var(--text-main, #0f172a); display: block;">{{ $plan->nombre }}</strong>
                    
                    <div style="margin: 14px 0 10px; display: flex; align-items: baseline; gap: 6px;">
                        <span style="font-size: 28px; font-weight: 900; color: var(--text-main, #0f172a);">{{ $plan->precio_formateado }}</span>
                        <span style="font-size: 12px; color: var(--text-muted, #64748b);">/ periodo</span>
                    </div>

                    <p style="font-size: 12.5px; color: var(--text-muted, #64748b); margin-bottom: 16px; line-height: 1.5;">
                        {{ $plan->descripcion ?? 'Sin descripción.' }}
                    </p>

                    <div style="font-size: 12px; color: var(--text-main, #334155); display: flex; flex-direction: column; gap: 8px; padding: 12px; background: var(--bg-page, #f8fafc); border-radius: 12px; border: 1px solid var(--border);">
